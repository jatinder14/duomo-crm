...
function NXBootCards() {
  ...
  if ($("#cardModal").length) {
    ...
    tinymce.init({
      ...
      height: 200,
      toolbar:
        "bold link bullist numlist alignleft aligncenter alignright image hr table code table fullscreen",
...
function NXPostGeneralComment() {
  ...
  tinymce.init({
    ...
    height: 200,
    toolbar:
      "formatselect undo redo bold | image link | alignleft aligncenter alignright outdent indent bullist numlist | hr table blockquote code",
    //autosave/update text area
    setup: function (editor) {
      editor.on("change", function () {
        editor.save();
...
function NXArticleCreate() {
  ...
  $(document)
    .off("select2:select", "#knowledgebase_categoryid")
    .on("select2:select", "#knowledgebase_categoryid", function (e) {
      //hide all
...
function NXProjectDetails() {
  ...
  //save button clicked
  $(document)
    .off("click", "#project-description-button-save")
    .on("click", "#project-description-button-save", function () {
      try {
...
function NXAddEditProject() {
  ...
  var project_progress = $("#js-projects-modal-add-edit").attr(
  ...
  $(document)
    ...
    .on("select2:select", "#project_template_selector", function (e) {
      //set new data
      $("#project_title").val($(this).find(":selected").attr("data-title"));

      $("#project_categoryid").val(
        $(this).find(":selected").attr("data-category")
      );
      $("#project_categoryid").trigger("change");

      $("#assignedperm_tasks_collaborate").prop(
        "checked",
        $(this).find(":selected").attr("data-client-task-collaborate") == "yes"
          ? true
          : false
      );
      $("#clientperm_tasks_view").prop(
        "checked",
        $(this).find(":selected").attr("data-client-task-view") == "yes"
          ? true
          : false
      );
      $("#clientperm_tasks_collaborate").prop(
        "checked",
        $(this).find(":selected").attr("data-client-task-collaborate") == "yes"
          ? true
          : false
      );
      $("#clientperm_tasks_create").prop(
        "checked",
        $(this).find(":selected").attr("data-client-task-create") == "yes"
          ? true
          : false
      );
      $("#clientperm_timesheets_view").prop(
        "checked",
        $(this).find(":selected").attr("data-client-view-timesheets") == "yes"
          ? true
          : false
      );
      $("#clientperm_expenses_view").prop(
        "checked",
        $(this).find(":selected").attr("data-client-view-expenses") == "yes"
          ? true
          : false
      );
      $("#project_billing_rate").val(
        $(this).find(":selected").attr("data-billing-rate")
      );
      $("#project_billing_type").val(
        $(this).find(":selected").attr("data-billing-type")
      );
      $("#project_billing_type").trigger("change");

      $("#project_billing_estimated_hours").val(
        $(this).find(":selected").attr("data-billing-estimated-hours")
      );
      $("#project_billing_costs_estimate").val(
        $(this).find(":selected").attr("data-billing-estimated-cost")
      );
...
function NXAddEditProjectTemplate() {
  ...
  $(document).on("click", "#commonModalSubmitButton", function () {
    if ($("#template_billing_rate").length) {
      ...
      $("#template_billing_rate").val(
        template_billing_rate.replace(/[^0-9.]/g, "")
      );
    }
  });
}
...
function NXCardsAssignedSelect() {
  ...
  $(document)
    .find(".card-assigned-listed-user")
    .each(function () {
...
function NXClientDetails() {
  ...
  $(document).on("click", "#client-description-button-edit", function () {
    ...
    nxTinyMCEExtendedLite(
      NX.client_description_height,
      NX.client_descrition_selector
    );
  });
  ...
  //save button clicked
  $(document)
    .off("click", "#client-description-button-save")
    .on("click", "#client-description-button-save", function () {
      try {
        $("#description").val(tinymce.activeEditor.getContent());
      } catch (err) {}
      //unbind events
...
function NXWebmailComposeEmail() {
  ...
  //select template
  $(document)
    .off("select2:select", "#email_template_selector")
    .on("select2:select", "#email_template_selector", function (e) {
      //make ajax request
      nxAjaxUxRequest($(this).find(":selected"));
    });
}
...
function NXUUpdateConverImage() {
  ...
  $("#fileupload_cover_image").dropzone({
    ...
    success: function (file, response) {
      ...
      //create a hidden form field for this file
      $preview.append(
        '<input type="hidden" name="cover_filename"  value="' +
          response.filename +
          '">'
      );
      $preview.append(
        '<input type="hidden" name="cover_directory"  value="' +
          response.uniqueid +
          '">'
      );
    },
  ...
  $("#settings-estimates-automation").validate({
    rules: {
      settings2_estimates_automation_project_title: {
        required: function (element) {
          return $("#settings2_estimates_automation_create_project").is(
            ":checked"
          );
        },
      },
      settings2_estimates_automation_invoice_due_date: {
        required: function (element) {
          return $("#settings2_estimates_automation_create_invoice").is(
            ":checked"
          );
        },
      },
