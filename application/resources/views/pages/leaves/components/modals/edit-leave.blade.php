<div class="row">
    <div class="col-lg-12">
            ...
            <div class="form-group row">
                ...
                <div class="col-sm-12 col-lg-9">
                    <select name="leave_creatorid" id="leave_creatorid"
                        ...
                        <option value="{{ $leave->leave_creatorid ?? '' }}">
        ...
        @else
            <input type="hidden" name="leave_creatorid" id="leave_creatorid"
                value="{{ $leave->leave_creatorid ?? '' }}">
        @endif
        <div class="form-group row">
            <label
                class="col-sm-12 col-lg-3 text-left control-label col-form-label required">{{ cleanLang(__('lang.date_from')) }}*</label>
            <div class="col-sm-12 col-lg-9">
                <input type="date" value="{{ $leave->leave_date_from ?? '' }}" class="form-control form-control-sm"
                    placeholder="@lang('lang.date_from')" name="leave_date_from" id="leave_date_from">
            </div>
        </div>
        <div class="form-group row">
            <label
            ...
            <div class="col-sm-12 col-lg-9">
                <input type="date" value="{{ $leave->leave_date_to ?? '' }}" class="form-control form-control-sm"
                    placeholder="@lang('lang.date_to')" name="leave_date_to" id="leave_date_to">
            </div>
        </div>
        <div class="form-group row">
            <label
                ...
                <select class="select2-basic form-control form-control-sm" id="leave_categoryid"
                    ...
                    @foreach ($categories as $category)
                        <option value="{{ $category->leavecategory_id }}"
                            {{ runtimePreselected($leave->leave_categoryid ?? '', $category->leavecategory_id) }}>
        ...
        <div class="form-group row">
            ...
            <div class="col-sm-12 col-lg-9">
                <textarea class="form-control form-control-sm" placeholder="@lang('lang.reason')" name="leave_reason" id="leave_reason">{{ $leave->leave_reason }}</textarea>
            </div>
        </div>
        <div class="form-group row">
            <label
                ...
                <select class="select2-basic form-control form-control-sm" id="leave_status" name="leave_status">
                    @foreach ($statuses as $status)
                        <option value="{{ $status->leavestatus_id }}"
                            {{ runtimePreselected($leave->leave_status ?? '', $status->leavestatus_id) }}>
                            {{ $status->leavestatus_title }}</option>
                    @endforeach
                </select>
            </div>
        </div>
        <div class="form-group row">
            ...
            <div class="col-sm-12 col-lg-9">
                <textarea class="form-control form-control-sm" placeholder="@lang('lang.comment')" name="leave_comment" id="leave_comment">{{ $leave->leave_comment ?? '' }}</textarea>
            </div>
        </div>
    </div>
</div>
