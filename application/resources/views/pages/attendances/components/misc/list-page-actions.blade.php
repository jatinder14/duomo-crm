...
<div class="col-md-12  col-lg-7 p-b-9 align-self-center text-right {{ $page['list_page_actions_size'] ?? '' }} {{ $page['list_page_container_class'] ?? '' }}"
    ...
    <div id="list-page-actions">
        ...
        @if (config('visibility.list_page_actions_importing'))
            <button type="button" title="{{ cleanLang(__('lang.import_attendances')) }}" id="attendances-import-button"
                ...
                data-action-url="{{ url('import/attendances') }}" data-action-method="POST"
                data-loading-target="commonModalBody" data-action-ajax-loading-target="commonModalBody"
                data-modal-title="@lang('lang.import_attendances')" data-url="{{ url('import/attendances/create') }}">
                <i class="ti-import"></i>
            </button>
        @endif
        ...
        @if (config('visibility.list_page_actions_exporting'))
            <button type="button" data-toggle="tooltip" title="@lang('lang.export_attendances')"
                class="list-actions-button btn btn-page-actions waves-effect waves-dark js-toggle-side-panel"
                data-target="sidepanel-export-attendances">
                <i class="ti-export"></i>
            </button>
        @endif
        ...
        @if (config('visibility.list_page_actions_add_button'))
            <button type="button"
                ...
                data-action-ajax-class="js-ajax-request" data-action-ajax-loading-target="attendances-td-container"
