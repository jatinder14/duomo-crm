...
class Destroy {
     ...
     *   <input type="checkbox" name="ids[{{ $holiday->id }}]"
     *
    ...
    public function handle($request, Closure $next) {
        ...
        if (is_numeric($request->route('holiday'))) {
            $ids[$request->route('holiday')] = 'on';
            request()->merge([
                'ids' => $ids,
            ]);
        }

        //loop through each holiday and check permissions
        if (is_array(request('ids'))) {
            ...
            foreach (request('ids') as $id => $value) {
                ...
                if (auth()->user()->is_team) {
                    if (auth()->user()->role->role_holidays >= 3) {
                        ...
                        if (auth()->user()->role->role_holidays_scope == 'own') {
                            if ($holiday->holidayr_creatorid != auth()->id()) {
            ...
            Log::error("no items were sent with this request", ['process' => '[permissions][holidays][change-category]', 'ref' => config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__, 'holiday id' => $id ?? '']);
            abort(409);
        }
