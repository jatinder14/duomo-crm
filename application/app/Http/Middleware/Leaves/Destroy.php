...
class Destroy {
     ...
     *   <input type="checkbox" name="ids[{{ $leave->id }}]"
     *
    ...
    public function handle($request, Closure $next) {
        ...
        if (is_numeric($request->route('leave'))) {
            $ids[$request->route('leave')] = 'on';
            request()->merge([
                'ids' => $ids,
            ]);
        }

        //loop through each leave and check permissions
        if (is_array(request('ids'))) {
                ...
                if (auth()->user()->is_team) {
                    if (auth()->user()->role->role_leaves >= 3) {
                        ...
                        if (auth()->user()->role->role_leaves_scope == 'own') {
                            if ($leave->leaver_creatorid != auth()->id()) {
            ...
            Log::error("no items were sent with this request", ['process' => '[permissions][leaves][change-category]', 'ref' => config('app.debug_ref'), 'function' => __function__, 'file' => basename(__FILE__), 'line' => __line__, 'path' => __file__, 'leave id' => $id ?? '']);
            abort(409);
        }
